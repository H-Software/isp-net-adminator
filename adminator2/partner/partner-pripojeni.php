...
<?php
    ...
    if ( ( !( ereg('^([[:digit:]])+$',$_GET["id_zadosti"]) ) and ( $_GET["odeslat"] == "OK" ) ) )
    {
    ...
    if ( ( !( ereg('^([[:digit:]])+$',$_GET["pripojeno"]) ) and ( $_GET["odeslat"] == "OK" ) ) )
    {
      echo "<div style=\"color: red; font-weight: bold; padding-left: 20px; padding-bottom: 20px; \">
    ...
    if ( ( !( ereg('^([[:digit:]])+$',$_GET["akt_tarif"]) ) and ( $_GET["odeslat"] == "OK" ) ) )
    {
      echo "<div style=\"color: red; font-weight: bold; padding-left: 20px; padding-bottom: 20px; \">
	...
	$akt_tarif = $_GET["akt_tarif"];
        ...
        $uprava=mysql_query("UPDATE partner_klienti SET pripojeno='$pripojeno', pripojeno_linka='$akt_tarif' 
	...
	echo "	<select name=\"id_zadosti\" size=\"5\" >
	 ...
	 $dotaz_zadosti=mysql_query("SELECT * FROM partner_klienti ORDER BY id DESC");
	 
	 while( $data=mysql_fetch_array($dotaz_zadosti) )
	 { 
