...
class vlastnik2_a2
{
    ...
    function vypis($sql,$co,$dotaz_final)
    {
	    ...
	    <td class=\"vlastnici-td-black\" colspan=\"6\" align=\"right\" width=\"\" >";
                ...
                // sem mazani    
                if($this->vlastnici_erase_povolen === false ) { $output .= "<span style=\"\" > smazat </span> "; 
                }
                else
                {
	...
	<td class=\"vlastnici-td-black\" >";
                ...
                // 6-ta update
                if($this->vlastnici_update_povolen === false ) { $output .= "<span style=\"\" >  upravit  </span> \n"; 
                }
                else
                {
                ...
                if($data["billing_freq"] == 0 ) { $output .= "měsíční"; 
                }
                elseif($data["billing_freq"] == 1 ) { $output .= "čtvrtletní"; 
                }
                else
                { $output .= "N/A"; 
                }
		...
		<div style=\"float: left; \">Pozastavené fakturace:</div>  ";
                ...
                if($data["billing_suspend_status"] == 1) { $output .= "Ano"; 
                }
                elseif($data["billing_suspend_status"] == 0) { $output .= "Ne"; 
                }
                ...
                if($data["billing_suspend_status"] == 1) {
                    ...
                    if((strlen($data["billing_suspend_start"]) > 0) or ($data["billing_suspend_start"] != null) ) { $output .= htmlspecialchars($data["billing_suspend_start_f"]); 
                    }
                    else
                    { $output .= "není zadáno"; 
                    }
                    ...
                    if((strlen($data["billing_suspend_stop"]) > 0) or ($data["billing_suspend_stop"] != null) ) { $output .= htmlspecialchars($data["billing_suspend_stop_f"]); 
                    }
                    else
                    { $output .= " není zadáno "; 
                    }
                    ...
                    if(strlen($data["billing_suspend_reason"]) == 0) { $output .= "není zadáno"; 
                    }
                    else
	 ...
	 tel: ".$data["telefon"]." </td>";
                ...
                if($data["sluzba_int"] == 1 ) { 
                    ...
                    if($data["sluzba_int_id_tarifu"] == 999 ) { $output .= "<span style=\"color: gray; \" >- tarif nezvolen</span></div>"; 
                    }
                    else
                ...
                if($data["sluzba_iptv"] == 1 ) { 
                    ...
                    if($data["sluzba_iptv_id_tarifu"] == 999 ) { $output .= "<span style=\"color: gray; \" >- tarif nezvolen</span></div>"; 
                    }
                    else
                ...
                if(( $sluzba_int_aktivni != 1 ) and ( $sluzba_iptv_aktivni != 1 ) and ( $sluzba_voip_aktivni != 1 ) ) { $output .= "<div style=\"color: Navy; font-weight: bold; \" >Žádná služba není aktivovaná</div>"; 
                }
                else{
                }
	    ...
	    <td colspan=\"10\" bgcolor=\"#99FF99\" >";
		    ...
		    <span style=\"\">vyberte akci: </span>
                ...
                $output .= "<option value=\"1\" "; if($_GET["akce"] == 1 ) { $output .= " selected ";
                } $output .= " > přiřadit objekt </option>";
                $output .= "<option value=\"15\" "; if($_GET["akce"] == 15 ) { $output .= " selected ";
                } $output .= " > přiřadit objekt STB</option>";

                   $output .= "</optgroup>";
        
                   $output .= "<optgroup label=\"fakturacni adresa\">";    
                $output .= "<option value=\"2\" "; if($_GET["akce"] == 2) { $output .= " selected ";
                } $output .= " >přidání fakturační adresy </option>";
                $output .= "<option value=\"3\" "; if($_GET["akce"] == 3) { $output .= " selected ";
                } $output .= " >smazání fakturační adresy </option>";
                $output .= "<option value=\"4\" "; if($_GET["akce"] == 4) { $output .= " selected ";
                } $output .= " >úprava fakturační adresy </option>";
                   $output .= "</optgroup>";
            
                   $output .= "<optgroup label=\"Závady/opravy\" >";
                $output .= "<option value=\"5\" "; if($_GET["akce"] == 5) { $output .= " selected ";
                } $output .= " >Vložit závadu/opravu</option>";
                $output .= "<option value=\"6\" "; if($_GET["akce"] == 6) { $output .= " selected ";
                } $output .= " >zobrazit závady/opravy</option>";
                   $output .= "</optgroup>";
            
                   $output .= "<optgroup label=\"Smlouvy/výpovědi\" >";
                $output .= "<option value=\"7\" "; if($_GET["akce"] == 7) { $output .= " selected ";
                } $output .= " >Tisk smlouvy</option>";
                $output .= "<option value=\"8\" "; if($_GET["akce"] == 8) { $output .= " selected ";
                ...
                $output .= "<option value=\"10\" "; if($_GET["akce"] == 10) { $output .= " selected ";
                } $output .= " >Výpis plateb za internet</option>";
                $output .= "<option value=\"11\" "; if($_GET["akce"] == 11) { $output .= " selected ";
                ...
                $output .= "<option value=\"16\" "; if($_GET["akce"] == 16) { $output .= " selected ";
                } $output .= " >Výpis faktur/Plateb (Pohoda SQL)</option>";        
        
                   $output .= "</optgroup>";
            
                   $output .= "<optgroup label=\"Historie\" >";
                $output .= "<option value=\"13\" "; if($_GET["akce"] == 13) { $output .= " selected ";
    ...
    {    
        ...
        if($this->export_povolen === true ) {
            ...
            if($fp === false) { echo "<div style=\"color: red; font-weight: bold; \">Chyba: Soubor pro export nelze otevřít </div>\n"; 
            }
            else
                {
