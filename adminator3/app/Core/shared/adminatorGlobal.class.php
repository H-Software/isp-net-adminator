...
class Aglobal
{
    ...
    function synchro_router_list()
    {
        ...
        foreach ($mysql_export as $key => $val) {
            if(ereg("^INSERT.", $val) ) { $mysql_export_all .= $val; 
            }
        }
    ...
    public function work_handler($item_id, $nothing = null)
    {
        ...
        if($count > 1) { echo "<div> WARNING: Požadavek na restart \"".$item_name."\" (No. ".$item_id.") nalezen vícekrát. </div>\n"; 
        }
        ...
        else
        {
            ...
            if($add) { echo "<span style=\"color: green;\"> úspěšně přidán do fronty</span>"; 
            }
            else
            { echo "<span style=\"color: red;\"> chyba při přidání požadavku do fronty</span>"; 
            }
    
            if($add_az) { echo " - <span style=\"color: green;\"> úspěšně přidán do archivu změn.</span>"; 
            }
            else
            { 
    ...
    */
    ...
    public static function create_link_to_owner($owner_id)
    {
        ...
        if ($archiv_vlastnik == 1) { $odkaz = "<a href=\"vlastnici-archiv.php?".urlencode("find_id")."=".urlencode($owner_id)."\" >".$owner_id."</a>\n"; 
        }
        else
