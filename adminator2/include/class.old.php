...
class objekt
{
 ...
 function zjistipocet($mod,$id)
 {
    ...
    $dotaz = pg_query("SELECT * FROM objekty WHERE ( id_cloveka = '$id' ".$tarif_sql." ) ");     
 ...
 function vypis($sql,$co,$id,$dotaz_final)
 {
   ...
   $list=$_GET["list"];
    ...
    { 
     ...
     $dotaz=pg_query($db_ok2,"SELECT * FROM objekty WHERE id_cloveka='$id' ".$tarif_sql); 
 ...
 if ($radku==0) 
 {
  ...
  {
   echo "<tr><td colspan=\"8\" ><span style=\"color: red; \">Nenalezeny žádné odpovídající data dle hledaného \"".$sql."\" ";
   ...
   //zacatek radny a prvni bunka
    ...
    global $update_povolen;
     ...
     $update_mod_vypisu = $_GET["mod_vypisu"];
      ...
      $dotaz_update = mysql_query("SELECT * FROM tarify_int WHERE id_tarifu = '$id_tarifu' ");
    ...
    { echo "<td class=\"tab-objekty2\" style=\"font-size: 10px; font-family: arial; color: grey;\">Upravit</td> \n"; }
    else
    {
    ...
    { echo "<td class=\"tab-objekty2\" style=\"font-size: 10px; font-family: arial; color: grey;\">Smazat</td>"; }
    else
    { 
    ...
    { $sikana_status_s="<font color=\"#CC3333\" >Sikana-A ( ".$data["sikana_cas"]." ) </font>"; } 
    else{ $sikana_status_s="<font color=\"green\">Sikana-N</font>"; }
	...
	echo "<a href=\"objekty-test.php?id_objektu=".$data["id_komplu"]."\" >test</a>";
      ...
      echo "<td class=\"tab-objekty2\" >";
       ...
       $tarif_f = mysql_query("SELECT * FROM tarify_int WHERE id_tarifu = '$id_tarifu' ");
	...
	{ 
	    ...
	    if( $data_f["id_tarifu"] == "1" )
	    {
		echo "<a class=\"tarifsc\" href=\"/monitoring/data/cat_sc.php?ip=".$data["ip"]."\" target=\"_blank\" >";
		echo $data_f["zkratka_tarifu"];	
		echo "</a>";
	    }
	    else
	    { echo $data_f["zkratka_tarifu"]; }
    ...
    $vysledek_bod = mysql_query("select * from nod_list WHERE id=".$id_nodu );
    $radku_bod = mysql_num_rows($vysledek_bod);
				      
     if($radku_bod==0) echo "<span style=\"color: grey; \">přípojný bod nelze zjistit </span>";
     else
     {
    ...
    { echo "V: <a href=\"vlastnici.php?find_id=".$data["id_cloveka"]."\" >".$data["id_cloveka"]."</a> </span> </td> \n"; }
    ...
    <!--  <form method=\"POST\" action=\"/adminator3/print/reg-form.php\" >
        <input type=\"hidden\" name=\"id_objektu\" value=\"".$data["id_komplu"]."\" >
    ...
    { 
     ...
     {
	echo "<td colspan=\"4\" style=\"font-size: 10px; font-family: arial; color: grey; \">
    ...
    elseif( $co==4 ) //opticky rezim
    {
     ...
     {
	echo "<td colspan=\"\" style=\"font-size: 10px; font-family: arial; color: grey; \">
	<div style=\"text-align: center; \">odendat</div> </td> \n"; 
     }
    }
   ...
   function vypis_tab ($par)
	...
	echo "<tr><td colspan=\"14\"> <br> </td> </tr>
		...
		global $vlastnici_update_povolen;
		 ...
		 { echo "<span style=\"color grey;\" >  upravit  </span> \n"; }
		else
		 { 
...
*/    
    ...
    echo "<a href=\"platby-vypis.php?id_vlastnika=".$data["id_cloveka"]."\" > výpis plateb </a>"; 
    ...
    if ( ( $data["fakturacni"] > 0 ) )
    { echo "<a href=\"vlastnici2-erase-f.php?id=".$data["fakturacni"]."\" > smazání fakturační adresy </a>"; }
    else
    { echo " smazání fakturační adresy " ;}
    
    echo "<span style=\"margin: 25px; \" ></span>";
    
    if ( ( $data["fakturacni"] > 0 ) )
    { echo "<a href=\"vlastnici2-change-fakt.php?id=".$data["fakturacni"]."\" > úprava fakturační adresy </a>"; }
    else
    { echo " úprava fakturační adresy "; }
   ...
   function vypis ($sql,$co,$dotaz_final)
   {
	...
	if ( (strlen($data["datum_podpisu"]) > 0) )
	{
	 ...
	 echo "<td colspan=\"\" valign=\"top\" >";
	  ...
	  { 
	    echo "<div><span style=\"font-weight: bold; \"><span style=\"color: #00cbfc; \" >Služba IPTV</span> - aktivní </span>";
...
class vlastnikarchiv
{
   ...
   function vypis_tab ($par)
	...
	//tady dalsi radka asi
	    ...
	    echo "<span style=\"color: grey; \">přidání objektu</span>";
	    ...
	    echo "<span style=\"margin-left: 20px; \">
		    <a href=pohoda_sql/phd_list_fa.php?id_vlastnika=".$data["id_cloveka"]."\" > výpis plateb (Pohoda Online)</a>".
		  "</span>";
...
class objektypridani {
    ...
    function generujdata ($selected_nod, $typ_ip, $dns)
    {
     ...
     $vysl_ip=mysql_query("SELECT * FROM nod_list WHERE id=".$selected_nod );
	...
	while ($data_ip=mysql_fetch_array($vysl_ip) ):
	   ...
	   $umisteni_aliasu=$data_ip["umisteni_aliasu"];  
	 ...
	 $gen_ip_find=$a.".".$b.".".$c.".".$d."/16";
	     ...
	     if ( $msq_check_ip_radku == 0 ) { $c=10; $gen_ip=$a.".".$b.".".$c.".".$d; }
	     else 
	     {
										 
	      while (  $data_check_ip=pg_fetch_array($msq_check_ip) ) { $gen_ip=$data_check_ip["ip"]; }
...
class objektypridanifiber{
  ...
  function generujdata( $selected_nod,$id_tarifu )
  {
     ...
     // skusime ip vygenerovat
     $vysl_nod=mysql_query("SELECT * FROM nod_list WHERE id=".$selected_nod );
	...
	endwhile;
     ...
     $vysl_tarif = mysql_query("SELECT * FROM tarify_int WHERE id_tarifu = '$id_tarifu' ");
...
class stb
{
 ...
 function zjistipocetobj($id_cloveka)
 {
    $sql_sloupce = " id_stb, id_cloveka, mac_adresa, puk, ip_adresa, popis, id_nodu, sw_port, pozn, datum_vytvoreni ";
     ...
     $dotaz = mysql_query("SELECT ".$sql_sloupce." FROM objekty_stb WHERE id_cloveka = '$id_cloveka' ORDER BY id_stb");
 ...
 function vypis($mod,$id_cloveka)
 {
    ...
    if( $mod == 1 )
    {
     $dotaz_vypis = mysql_query("SELECT ".$sql_sloupce." FROM objekty_stb WHERE id_cloveka = '$id_cloveka' ORDER BY id_stb");
     $dotaz_vypis_radku = mysql_num_rows($dotaz_vypis);
    }
    else
    {
     $dotaz_vypis = mysql_query("SELECT ".$sql_sloupce." FROM objekty_stb ORDER BY id_stb");
     $dotaz_vypis_radku = mysql_num_rows($dotaz_vypis);
    }
	...
	echo "<tr><td colspan=\"".$pocet_sloupcu."\" >
	...
	while($data_vypis = mysql_fetch_array($dotaz_vypis))
	{
	  ...
	  echo "
	    <tr>
    	    <td>".$data_vypis["id_stb"]."</td>";
    	    //<td>".$data_vypis["id_cloveka"]."</td>";
		...
		$archiv_vlastnik=$data_vlastnik["archiv"]; 
	      ...
	      if ( $archiv_vlastnik == 1)
	      { $odkaz = "<a href=\"vlastnici-archiv.php?find_id=".$data_vypis["id_cloveka"]."\" >".$data_vypis["id_cloveka"]."</a>\n"; }
	      elseif ($firma_vlastnik == 1 )
	      { $odkaz = "<a href=\"vlastnici2.php?find_id=".$data_vypis["id_cloveka"]."\" >".$data_vypis["id_cloveka"]."</a>\n"; }
	      else
	      { $odkaz = "<a href=\"vlastnici.php?find_id=".$data_vypis["id_cloveka"]."\" >".$data_vypis["id_cloveka"]."</a>\n"; }

    	echo "<td>".$odkaz."</td>
	    <td>".$data_vypis["mac_adresa"]."</td>
    	    <td>".$data_vypis["ip_adresa"]."</td>
    	    <td>".$data_vypis["puk"]."</td>

    	    <td>".$data_vypis["popis"]."</td>

    	    <td>".$data_vypis["id_nodu"]."</td>
    	    <td>".$data_vypis["sw_port"]."</td>

    	    <td>";

    	    $pridano = $data_vypis["datum_vytvoreni"];

    	    // druha bunka - pozn
    	    echo "<span class=\"pozn\"><img title=\"poznamka\" src=\"img2/poznamka3.png\" align=\"middle\" ";
    	    echo " onclick=\"window.alert(' poznámka: ".$data_vypis["pozn"]." , Vytvořeno: ".$pridano." ');\" ></span>\n";

    	    echo "</td>
		    
		<td>
		 <a href=\"objekty-test.php?id_stb=".$data_vypis["id_stb"]."\" >test</a>
		</td>
    		<td>";
	    ...
	    global $level;
    	    ...
    	    { echo "<div style=\"color: gray; \" >úprava</div>"; }
    	    else
    	    {
    		echo "<form method=\"POST\" action=\"objekty-stb-add.php\" >
        	<input type=\"hidden\" name=\"update_id\" value=\"".$data_vypis["id_stb"]."\" >
...
class Aglobal
{
	...
	$count = mysql_result(mysql_query("SELECT COUNT(*) FROM workitems WHERE number_request = '$item_id' "), 0);
