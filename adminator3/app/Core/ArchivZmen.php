...
class ArchivZmen
{
    ...
    public function mutateStbParams(array $dataOrig, array $dataUpdated)
    {
        $pole3 = "";
    ...
    public function insertItem(int $actionType, array $actionData, int $actionResult = 0, string $loggedUserEmail = null)
    {
    ...
    public function insertItemDiff(int $actionType, array $dataOrig, array $dataUpdated, ...$args)
    {
    ...
    public function archivZmenList()
    {
        $output = "";
        ...
        if(!isset($id_nodu) and !isset($id) and !isset($id_stb) and !isset($id_cloveka) and !isset($id_routeru)) {
            ...
            if($typ == "1") {
                $output .= " selected ";
            } $output .= " >akce: úprava objektu</option>
            <option value=\"2\"";
            if($typ == "2") {
                $output .= " selected ";
            } $output .= " >akce: přidání objektu</option>
            <option value=\"3\"";
            if($typ == "3") {
                $output .= " selected ";
            ...
            if($typ == "4") {
                $output .= " selected ";
            } $output .= " >akce: úprava stb objektu</option>
            <option value=\"5\"";
            if($typ == "5") {
                $output .= " selected ";
            } $output .= " >akce: přidání stb objektu</option>
            <option value=\"6\"";
            if($typ == "6") {
                $output .= " selected ";
            ...
            if($typ == "7") {
                $output .= " selected ";
            } $output .= " >akce: úprava vlastníka</option>
            <option value=\"8\"";
            if($typ == "8") {
                $output .= " selected ";
            ...
            if($typ == "9") {
                $output .= " selected ";
            } $output .= " >akce: přidáni fakturační adresy</option>
            <option value=\"10\"";
            if($typ == "10") {
                $output .= " selected ";
            } $output .= " >akce: úprava fakturační adresy</option>
            <option value=\"11\"";
            if($typ == "11") {
                $output .= " selected ";
        ...
        <!-- akce: poslani SMS z duvodu neplacení -->
            ...
            if($typ == "12") {
                $output .= " selected ";
            } $output .= " >akce: přiřazení objektu k vlastníkovi</option>
            <option value=\"13\"";
            if($typ == "13") {
                $output .= " selected ";
            } $output .= " >akce: prirazeni objektu typu STB k vlastnikovi</option>

            <option value=\"14\"";
            if($typ == "14") {
                $output .= " selected ";
            } $output .= " >akce: odrazeni objektu (od vlastníka)</option>
            <option value=\"15\"";
            if($typ == "15") {
                $output .= " selected ";
            } $output .= " >akce: odparovani stb objektu (od vlastníka)</option>
        ...
        <!-- akce: automaticke nastaveni sikany z duvodu neuhr. faktur -->
            ...
            if($typ == "17") {
                $output .= " selected ";
            } $output .= " >akce: přidání routeru</option>
            <option value=\"18\"";
            if($typ == "18") {
                $output .= " selected ";
            } $output .= " >akce: úprava routeru</option>
            <option value=\"19\"";
            if($typ == "19") {
                $output .= " selected ";
            } $output .= " >akce: smazání routeru</option>
            ...
            if($typ == "20") {
                $output .= " selected ";
            } $output .= " >akce: přidání nodu</option>
            <option value=\"21\"";
            if($typ == "21") {
                $output .= " selected ";
            } $output .= " >akce: úprava nodu</option>
            <option value=\"22\"";
            if($typ == "22") {
                $output .= " selected ";
            } $output .= " >akce: smazání nodu</option>
            ...
            if($typ == "23") {
                $output .= " selected ";
            } $output .= " >akce: přidání/změna grafu</option>
            <option value=\"24\"";
            if($typ == "24") {
                $output .= " selected ";
            } $output .= " >akce: smazání grafu</option>
            ...
            while ($data = $vysl->fetch_array()):
                ...
                */
                elseif(preg_match("/odrazeni objektu/", $akce)) {
                    ...
                    if(preg_match('/^([[:digit:]]+)$/', $id_nodu_pomocne)) {
                        ...
                        $akce = preg_replace("/ ".$id_nodu_pomocne." /", " ".$id_nodu_rs." ", $akce);
                    }
                    ...
                    $akce = preg_replace("/href=\"topology-router-list.php\"/", "href=\"topology-router-list.php?f_id_routeru=".intval($id_routeru_pomocne)."&odeslano=OK\"", $akce);
                }

                $output .=  $akce."</span>\n</td>\n";

                $output .= "<td class=\"az-border1\"><span class=\"az-provedeno-kdy\" >";
                if ((strlen($data["provedeno_kdy2"]) < 1)) {
                    $output .= "&nbsp;";
                } else {
                    $output .=  $data["provedeno_kdy2"];
                }
                $output .= "</span></td>\n";

                $output .= "<td class=\"az-border1\"><span class=\"az-provedeno-kym\" >";
                if ((strlen($data["provedeno_kym"]) < 1)) {
                    $output .= "&nbsp;";
                } else {
                    $output .=  $data["provedeno_kym"];
                }
                $output .= "</span></td>\n";

                $output .= "<td class=\"az-border1\">";
                if ($data["vysledek"] == 1) {
                    $output .= "<span class=\"az-vysl-ano\">Ano</span>";
                } else {
                    $output .= "<span class=\"az-vysl-ne\">Ne</span>";
                }
                $output .= "</td>\n";
    ...
    public function archivZmenWork()
    {
        $output = "";
        ...
        if ($radku == 0) {
            ...
            while ($data = $vysl->fetch_array()):
                ...
                if ($data["vysledek"] == 1) {
                    ...
                    $output .= "<span class=\"az-vysl-ne\">&nbsp;</span>";
                }
                $output .= "</td>";
