...
class mk_net_n_sikana
{
    ...
    function find_root_router($id_routeru, $ip_adresa_routeru)
    {
        ...
        { //ani jedno predchozi, rekurze .. :)
            if($this->find_root_router($parent_router, $ip_adresa_routeru) == true) { return true; 
            }
        }
    ...
    function find_obj($ip)
    {
        ...
        while($data_routers = $rs_routers->fetch_array())
        {
            ...
            if($this->find_root_router($id_routeru, $ip) === true) { $routers[] = $id_routeru; 
            }
        }
        ...
        while( $data = pg_fetch_array($this->rs_objects))
        {
            ...
            if($data["dov_net"] == "n") { $this->objects_net_n[] = $data["ip"]; 
            }
            elseif($data["sikana_status"] == "a" ) { $this->objects_sikana[] = $data["ip"]; 
            }
            else
        ...
        if($this->debug == 1) { echo ", array objects counts: ".count($this->objects_net_n)." ".count($this->objects_sikana); 
        }
    ...
    function detect_diff_and_repaid($mod)
    {
        ...
        if($mod == "net-n") { $system_items = $this->objects_net_n; 
        }
        else
        ...
        foreach ($responseFwAddrList as $key => $value) {
            ...
            if($this->getall["$key"]["list"] == "$mod") {
                ...
                if($this->getall["$key"]["disabled"] == "true" ) { $this->wrong_items[] = $id; 
                }
                else
        ...
        if(((count($this->arr_diff_exc) == 0) and (count($this->arr_diff_mis) == 0) and (count($this->wrong_items) == 0) ) ) { echo "  $mod: records OK <br>\n"; 
        }
        else
        {
            ...
            if((count($this->wrong_items) > 0) ) { $this->remove_wrong_items($this->wrong_items); 
            }
