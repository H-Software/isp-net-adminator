...
class stb extends adminator
{
    ...
    public function stbListGetBodyContent()
    {
        ...
        if((strlen($_GET["list"]) > 0) ) { $list = intval($_GET["list"]); 
        }
        
        if((strlen($_GET["id_stb"]) > 0) ) { $id_stb = intval($_GET["id_stb"]); 
        }
        ...
        $output .= "<div style=\"float: left; \">".    
                ...
                "<option value=\"1\" "; if($par_vlastnik == 1) { $output .= " selected ";
                } $output .= ">Ano (spárované)</option>".
                "<option value=\"2\" "; if($par_vlastnik == 2) { $output .= " selected ";
    ...
    function stbActionSaveIntoDatabase($data)
    {
        if($data['update_id']) {
            ...
            if($res) { $output .= "<H3><div style=\"color: green;\" >Data úspěšně uloženy.</div></H3>\n"; 
            } 
            else
            { 
            ...
            if(is_object($azRes) ) { $output .= "<br><H3><div style=\"color: green;\" >Změna byla úspěšně zaznamenána do archivu změn.</div></H3>\n"; 
            } 
            else
