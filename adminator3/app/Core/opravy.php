...
class opravy
{
    ...
    function vypis_opravy($pocet_bunek)
    {
        ...
        while($data=$dotaz->fetch_array() )
        {
            ...
            if($v_reseni_filtr == 1 ) {          
                if (( ($data["v_reseni"] == 1) and ( $dotaz_radku_S1 == 0 ) ) ) { $zobrazovat="ano"; 
                }
            ...
            elseif($v_reseni_filtr == 0 ) {                  
                if(( ($data["v_reseni"] == 0 ) and ( $dotaz_radku_S1 == 0 ) ) ) { $zobrazovat="ano"; 
                }
                elseif ($dotaz_radku_S1 > 0 ) { 
                    while($data_S1=$dotaz_S1->fetch_array() )
                    { 
            ...
            if($vyreseno_filtr == 1) {
                ...
                if (( ($data["vyreseno"] == 1) and ( $dotaz_radku_S1 == 0 ) ) ) { $zobrazovat="ano"; 
                }
                elseif($dotaz_radku_S1 > 0 ) {
                    while($data_S1=$dotaz_S1->fetch_array() )
            ...
            elseif ($vyreseno_filtr == 0 ) {
                // prvne zjistime jestli jde o singl prispevek bo jestli je jich vic
                if (( ($data["vyreseno"] == 0) and ( $dotaz_radku_S1 == 0 ) ) ) { $zobrazovat="ano"; 
                }
                elseif($dotaz_radku_S1 > 0 ) {
                    while($data_S1=$dotaz_S1->fetch_array() )
                    { 
            ...
            if (( $v_reseni_filtr == 99 and $vyreseno_filtr == 99 ) ) { $zobrazovat="ano"; 
            }
            ...
            if(($zobrazovat == "ano" and $zobrazeno == "ne" and $exit != "ano" ) ) {
                ...
                if ($archiv_vlastnik == 1 ) { $this->vypis_opravy_content_html .= "<a href=\"". fix_link_to_another_adminator("/vlastnici-archiv.php?find_id=".$data["id_vlastnika"])."\" "; 
                }
                elseif ($firma_vlastnik == 1 ) { $this->vypis_opravy_content_html .= "<a href=\"". fix_link_to_another_adminator("/vlastnici2.php?find_id=".$data["id_vlastnika"])."\" "; 
                }
                else
            ...
            if(( $sekundarni_show == "ano" and $zobrazeno == "ne" ) ) {
                ...
                while($data_S2=$dotaz_S2->fetch_array() )
                {
                    ...
                    if ($zobrazeno == "ne" and $exit != "ano" ) {    
                        ...
                        if ($archiv_vlastnik == 1 ) { $this->vypis_opravy_content_html .= "<a href=\"" . fix_link_to_another_adminator("/vlastnici-archiv.php?find_id=".$data_S2["id_vlastnika"]) . "\" "; 
                        }
                        elseif ($firma_vlastnik == 1 ) { $this->vypis_opravy_content_html .= "<a href=\"" . fix_link_to_another_adminator("/vlastnici2.php?find_id=".$data_S2["id_vlastnika"]) . "\" "; 
                        }
                        else
